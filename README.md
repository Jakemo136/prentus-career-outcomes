...
- **Hero-metric override on `CoverageMeter`.** By default the bar uses threshold coloring (`bg-red-500` below 50%, `bg-orange-500` below 75%, `bg-green-500` at or above) for at-a-glance triage — the color IS the signal. A `tone="brand"` prop swaps to `bg-primary-500` (purple) regardless of percent, used for the Verified Earnings KPI so the hero metric looks authoritative rather than alarming at low coverage. Every value is a Figma-sourced anchor (red / orange / green / primary at the `-500` ramp stop) — no interpolation, no arbitrary hex.
